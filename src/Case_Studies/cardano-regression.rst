...
worker/wrapper'd but defines a :term:`join point` for the entire ``Just``
branch. Using a join point could be slower than inlining [#]_ because the join
point may prevent future optimizations that inlining would allow, which could be
the case here. However, we already have evidence that |old| is more performant
than |new|, which implies that the join point has a meaningful effect.
...
point. |old| generates the ``Just`` branch beginning with a ``let`` binding:
``addrHash_s7bbM``, which is used to construct *another* just. So we have
something like this:
...
Which is sub-optimal. This code unpacks a ``Just`` only to then re-allocate a
new ``Just``! So the join point optimizes that ``let`` and the redundant
un-packing and re-packing of the ``Just``. We should expect this optimization to
have an impact. It is in a branch that, according to the profile, could have
been entered 1000001 times, and likely was. Therefore avoiding the ``let`` and
its concomitant closure would save allocations.

We now have enough data to formulate a hypothesis: ``updateStakeDistribution``
regresses because ``decodeAddress28`` does not form a join point when compiling
with |new| leading to wasteful unpacking and re-packing of a ``Just``.

Join points only come from tail calls where two control flow branches meet or
*join*; in this case the only possible place the join point comes from
``paymentCred`` in the source code:

TODO:
- discuss where the Justs come from
- let that guide changes to the code
- show a join point on new
- show the unfolding on new, show that the redundant packing/repacking is not
  present and thus the join point isn't needed anymore. this means that the
...
[GblId,
 ...
 Unf=Unf{Src=InlineStable, TopLvl=True, Value=True, ConLike=True,
         ...
         Tmpl= \ (@c_aEqk)
                 ($dHashAlgorithm_aEql [Occ=Once1]
                    :: HashAlgorithm (ADDRHASH c_aEqk))
                 (eta_X1q [Occ=Once1] :: Credential 'Staking c_aEqk)
                 (eta1_X1r [Occ=Once1!] :: Addr28Extra) ->
                 case eta1_X1r of
                 { Addr28Extra dt_dNmB [Occ=Once2] dt1_dNmC [Occ=Once2]
                               dt2_dNmD [Occ=Once2] dt3_dNmE ->
                 case (Cardano.Crypto.Hash.Class.$p1HashAlgorithm
                         @(ADDRHASH c_aEqk) $dHashAlgorithm_aEql)
                      `cast` (GHC.TypeNats.N:KnownNat[0] <SizeHash (ADDRHASH c_aEqk)>_N
                              ; GHC.TypeNats.N:SNat[0] <SizeHash (ADDRHASH c_aEqk)>_P
                              :: KnownNat (SizeHash (ADDRHASH c_aEqk)) ~R# Natural)
                 of x1_aK2k [Occ=Once1]
                 { __DEFAULT ->
                 case GHC.Num.Natural.naturalEq# x1_aK2k 28 of {
                   __DEFAULT -> GHC.Maybe.Nothing @(Addr c_aEqk);
                   1# ->
                     case Unsafe.Coerce.unsafeEqualityProof
                            @(*)
                            @(ghc-prim:GHC.Types.Any :~: ghc-prim:GHC.Types.Any)
                            @(SizeHash (ADDRHASH c_aEqk) :~: 28)
                     of
                     { Unsafe.Coerce.UnsafeRefl co_aRX7 ->
                     GHC.Maybe.Just
                       @(Addr c_aEqk)
                       (Cardano.Ledger.Address.Addr
                          @c_aEqk
                          (case GHC.Word.neWord64
                                  (GHC.Word.W64# (ghc-prim:GHC.Prim.and# dt3_dNmE 2##))
                                  (GHC.Word.W64# 0##)
                           of {
                             False -> Cardano.Ledger.BaseTypes.Testnet;
                             True -> Cardano.Ledger.BaseTypes.Mainnet
                          ...
                          (case GHC.Word.neWord64
                                  (GHC.Word.W64# (ghc-prim:GHC.Prim.and# dt3_dNmE 1##))
                                  (GHC.Word.W64# 0##)
                           of {
                             False ->
                               Cardano.Ledger.Credential.$WScriptHashObj
                                 ...
                                 ((cardano-crypto-class-2.1.2.0-AvwSxmsDYBzI1yIWWm4yyw:Cardano.Crypto.PackedBytes.PackedBytes28
                                     ...
                                     dt2_dNmD
                                     (ghc-prim:GHC.Prim.wordToWord32#
                                        (ghc-prim:GHC.Prim.uncheckedShiftRL# dt3_dNmE 32#)))
                                  `cast` ((PackedBytes
                                             (Nth:2 (Sub co_aRX7)
                                              ; Nth:1 (Sub (Sym co_aRX7))))_R
                                          ; Sym (Cardano.Crypto.Hash.Class.N:Hash[0]
                                                     <ADDRHASH c_aEqk>_N <EraIndependentScript>_P)
                                          ; Sym (Cardano.Ledger.Hashes.N:ScriptHash[0] <c_aEqk>_N)
                                          :: PackedBytes 28 ~R# ScriptHash c_aEqk));
                             True ->
                               Cardano.Ledger.Credential.$WKeyHashObj
                                 @'Payment
                                 @c_aEqk
                                 ((cardano-crypto-class-2.1.2.0-AvwSxmsDYBzI1yIWWm4yyw:Cardano.Crypto.PackedBytes.PackedBytes28
                                  ...
                                  `cast` ((PackedBytes
                                          ...
                                          ; Sym (Cardano.Ledger.Keys.N:KeyHash[0]
                                                     <'Payment>_P <c_aEqk>_N)
                                          :: PackedBytes 28 ~R# KeyHash 'Payment c_aEqk))
                           })
                          (Cardano.Ledger.Credential.$WStakeRefBase @c_aEqk eta_X1q))
                     }
                 }
                 }
                 }}]
...
We can be confident that this is the source of the join point because |old|
established a join point for ``addrHash``
